...
if __name__ == "__main__":
  ...
  for image_id,SOURCE_IM_PATH in enumerate(SOURCE_IM_PATH_ARRAY):
    ...
    dets_faces,bboxes_faces = align_image_with_mtcnn_with_tf_graph(image_np, pnet, rnet, onet, image_size=IMAGE_SIZE)
    ...
    for i,bbox_face in enumerate(bboxes_faces):
      ...
      if best_class_probabilities[i] > 0.7:
        cv2.putText(image, class_names[best_class_indices[i]], (int(bbox_face[0]), int(bbox_face[1]) + 10), 0, 0.6, (0, 255, 0))
      else:
